...
function buildRequestListener(getProxyFunction, ipv4Only) {
    console.log("YYYYYYY", ipv4Only);
    return function (request, response) {
      console.log("ZZZZZZZZZZ", ipv4Only);
        ...
        resolve(ph.host, function(err, hostname) {
            if (err) {
                ...
                const proxyUrl = `socks5://${proxy.authentication.username}:${proxy.authentication.password}@${proxy.ipaddress}:${proxy.port}`;
...
function checkProxyForipv6(url) {
  console.log("XXXXXXXXXXXXXXXXXXX");
  const agent = new SocksProxyAgent(
    url
  );

  const req = http.get('http://ipv6.google.com', { agent }, (res) => {
